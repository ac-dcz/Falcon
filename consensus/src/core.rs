...
impl Core {
    ...
    async fn process_block(&mut self, block: &Block) -> ConsensusResult<()> {
        ...
        let (b0, b1) = match self.synchronizer.get_ancestors(block).await? {
            ...
            //如果暂时没有parent 那就先不提交
            //会向其他节点发送request (收到了BLOCK,说明至少有f+1个诚实节点收到 block 的 parent)
            None => {
        ...
        loop {
            let result = tokio::select! {
                Some(message) = self.core_channel.recv() => {
                    match message {
                        ConsensusMessage::Propose(block) => self.handle_proposal(&block).await, //收到propose
                        ConsensusMessage::Vote(vote) => self.handle_vote(&vote).await,      //处理收到的vote
                        ...
                        ConsensusMessage::LoopBack(block) => self.process_block(&block).await,  // 在处理Block
                        ConsensusMessage::SyncRequest(digest, sender) => self.handle_sync_request(digest, sender).await,//收到其他节点请求区块
                        ConsensusMessage::SyncReply(block) => self.handle_proposal(&block).await,   //收到区块后存储
                    }
                },
