...
impl fmt::Debug for Vote {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        write!(
            f,
            "Vote(blockhash {}, proposer {}, view {}, round {}, height {}, fallback {}, voter {})",
            self.hash,
            self.proposer,
            self.view,
            self.round,
...
impl fmt::Debug for QC {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        write!(
            f,
            "QC(hash {}, view {}, round {}, height {}, fallback {}, proposer {})",
            self.hash, self.view, self.round, self.height, self.fallback, self.proposer
        )
    }
}
...
impl fmt::Debug for Timeout {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        write!(
            f,
            "TV(author {}, view/round {}, highqc {:?})",
            self.author, self.seq, self.high_qc
        )
    }
}
...
impl TC {
    pub fn verify(&self, committee: &Committee) -> ConsensusResult<()> {
        ...
        for (name, _, _) in self.votes.iter() {
                ...
                !used.contains(name),
                ConsensusError::AuthorityReuseinTC(*name)
            );
            let voting_rights = committee.stake(name);
            ensure!(voting_rights > 0, ConsensusError::UnknownAuthority(*name));
            used.insert(*name);
